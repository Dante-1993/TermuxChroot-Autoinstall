...
download_and_execute_script() {
    ...
    if [ -e "/data/local/tmp/start_debian.sh" ]; then
        ...
        wget -O "/data/local/tmp/start_debian.sh" "https://raw.githubusercontent.com/Dante-1993/TermuxChroot-Autoinstall/refs/heads/main/start-debian.sh"
        if [ $? -eq 0 ]; then
...
configure_debian_chroot() {
    ...
    grep -q 'AUTO_START_SERVICES' \$BASHRC 2>/dev/null || tee -a $BASHRC << EOF 
...
download_startxfce4_script() {
    ...
    if [ "$DE_OPTION" -eq 1 ]; then
        wget -O "./startxfce4_chrootDebian.sh" "https://raw.githubusercontent.com/Dante-1993/TermuxChroot-Autoinstall/refs/heads/main/startxfce4_chrootDebian.sh"
        if [ $? -eq 0 ]; then
